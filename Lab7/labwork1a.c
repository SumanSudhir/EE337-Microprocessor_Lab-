...
unsigned long adcVal=0,tempadcVal=0 ,temp_low,temp_high,try0=0;
...
void main(void)
{
	...
	while(1)
	{
		...
		CS_BAR = 1;
		tempadcVal = (data_save_high<<2) + (data_save_low>>6);//-right		// save the 10 bits in one variable
	}
}
/**
...
void timer0_ISR (void) interrupt 1
{
  ...
  TL0 = 0XB0;//Initialize TL0  sbit TR0}
	TR0 = 1;

  z = z+1;
	voltage = (3.3*tempadcVal*1000)/1024;
	sum = sum+voltage;
  if(z%15==0){
    z = 0;
		sum = sum/15;
    ...
    digit[3] = '0' +  sum%10;
    digit[2] = '0' + (sum/10)%10;
    digit[1] = '0' + (sum/100)%10;
    digit[0] = '0' + (sum/1000)%10;
    LCD_Ready();
    LCD_CmdWrite(0x85);
    LCD_StringWrite(digit,4);
...
void Timer_Init()
{
  TMOD = 0x01;// Set Timer0 to work in up counting 16 bit mode. Counts upto
